...
function SettingsUniversalConnector() {
  ...
  return <SettingsSubpage tab="connector"><div className="settings-section-title">Universal Connector</div><p className="muted">A local gateway for sensors, services, and future actuators. It keeps external tools separate from the cognitive core and does not connect until enabled.</p><div className="settings-form settings-form-compact"><label className="settings-field"><span>Enable Universal Connector</span><input type="checkbox" checked={Boolean(values.UNIVERSAL_CONNECTOR_ENABLED)} onChange={e => set('UNIVERSAL_CONNECTOR_ENABLED', e.target.checked)}/></label><label className="settings-field"><span>Enable Home Assistant bridge</span><input type="checkbox" checked={Boolean(values.HOME_ASSISTANT_ENABLED)} onChange={e => set('HOME_ASSISTANT_ENABLED', e.target.checked)}/></label><label className="settings-field"><span>Home Assistant URL</span><input value={values.HOME_ASSISTANT_URL || ''} onChange={e => set('HOME_ASSISTANT_URL', e.target.value)} placeholder="http://homeassistant.local:8123"/></label><label className="settings-field"><span>Long-lived access token</span><input type="password" value={values.HOME_ASSISTANT_TOKEN === '••••••••' ? '' : (values.HOME_ASSISTANT_TOKEN || '')} onChange={e => set('HOME_ASSISTANT_TOKEN', e.target.value)} placeholder="Stored securely in config.json"/></label><label className="settings-field"><span>Verify HTTPS certificate</span><input type="checkbox" checked={values.HOME_ASSISTANT_VERIFY_SSL !== false} onChange={e => set('HOME_ASSISTANT_VERIFY_SSL', e.target.checked)}/></label><label className="settings-field"><span>Polling interval (seconds)</span><input type="number" min="1" max="300" value={values.HOME_ASSISTANT_POLL_INTERVAL ?? 5} onChange={e => set('HOME_ASSISTANT_POLL_INTERVAL', Number(e.target.value))}/></label><label className="settings-field wide"><span>Allowed Home Assistant domains</span><input value={values.HOME_ASSISTANT_ALLOWED_DOMAINS || ''} onChange={e => set('HOME_ASSISTANT_ALLOWED_DOMAINS', e.target.value)} placeholder="person,device_tracker,sensor,binary_sensor,zone"/></label></div><div className="provider-notes"><div>HOME ASSISTANT</div><p>Use the NAS Home Assistant address and create a Long-Lived Access Token in the Home Assistant profile.</p><p>Initially, Lumina will treat permitted entities as perceptions. Actuator control will remain separately permissioned and auditable.</p></div><div className="lumina-actions"><button onClick={() => void save()}><Check size={15}/>Save connector settings</button><span>{message || error}</span></div></SettingsSubpage>;
}
