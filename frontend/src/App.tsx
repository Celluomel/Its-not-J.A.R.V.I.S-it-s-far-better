...
type Message = { id: string; role: 'user' | 'assistant'; text: string; stopped?: boolean; feedback?: 'positive' | 'negative'; telemetry?: CognitiveTelemetry };
...
function CapabilityExperimentPanel({ proposalId }: { proposalId: string }) {
  ...
  const baselineCount = experiment.baseline?.length || 0; const trialCount = experiment.trials?.length || 0;
  return <div className="development-experiment"><div className="settings-object-grid"><div><span>Experiment state</span><b>{experiment.status}</b></div><div><span>Baseline</span><b>{baselineCount}/{experiment.max_baseline_steps}</b></div><div><span>Trials</span><b>{trialCount}/{experiment.max_trial_steps}</b></div><div><span>Subsystem</span><b>{experiment.subsystem}</b></div></div>{experiment.result && <div className="settings-analysis"><b>Evidence</b><p>Baseline {experiment.result.baseline_score} → trials {experiment.result.trial_score} · improvement {experiment.result.improvement}</p><p>{experiment.result.evidence}</p></div>}{state?.verified_capability && <div className="settings-analysis"><b>Verified capability</b><p>{state.verified_capability.name} · {state.verified_capability.status}</p></div>}</div>;
}
