...
def _get_competition_instance(global_workspace=None):
    """
    Returns the dedicated WorkspaceCompetition instance for this module
    (see module docstring for why it's kept separate from
    Phase1Orchestrator's own instance).

    Bug fix: previously constructed with no global_workspace reference at
    all, so WorkspaceCompetition.compete()'s publish step (workspace_
    competition.py ~495-506 — writes focus/uncertainty/hypotheses into
    GlobalWorkspace) silently never ran on this, the actual live per-turn
    path. Only the periodic background Phase1Orchestrator instance
    (core/phase1_integration.py) was ever wired with one — which is why
    "workspace_competition" never showed up in the dashboard's
    broadcast_participants, and Current focus/Uncertainty stayed
    permanently blank, despite deliberate() running every single turn.
    Now threads through the same organism.workspace deliberate() already
    fetches below.
    """
    ...
    if _deliberation_competition is None:
        ...
        _deliberation_competition = WorkspaceCompetition(global_workspace=global_workspace)
    elif global_workspace is not None and _deliberation_competition._global_workspace is None:
        # Instance was created before an organism reference was available
        # (e.g. an early warmup call) — attach it now instead of staying
        # unwired for the rest of the process lifetime.
        _deliberation_competition._global_workspace = global_workspace
...
def deliberate(
    ...
    competition = _get_competition_instance(gw)
