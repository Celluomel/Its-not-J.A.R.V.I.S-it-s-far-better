...
Single-call mode (current): build_inline_directive() folds the reasoning
instruction into the SAME generation as the real response — one LLM call,
not two. The old run() (true two-pass, 2x LLM calls) is kept below as a
fallback path but is no longer on the hot path from persona_bridge.

Integration:
    build_inline_directive()/capture_reasoning() called from
    persona_bridge._stream_with_full_lifecycle().
...
mechanically reproduce it, just let it guide you."""
    ...
    # ── Single-call mode ─────────────────────────────────────────────────────
    # The fields run()/_build_directive() derive from the LLM's own
    # DRIVE/STYLE/etc. self-report (drive_alignment → temperature, depth,
    # identity_anchor) are all already computable directly from organism
    # state, with no LLM involved. build_inline_directive() does that, then
    # folds a <think> instruction into the SAME generation persona_bridge is
    # about to run, instead of spending a separate blocking call on it first.
    # capture_reasoning() parses the <think> block back out of the stream
    # afterward, so last_reasoning()/summary() keep working unchanged.

    def _dominant_drive_name(self) -> str:
        """Same lookup _build_cognitive_context() uses — no LLM involved."""
        o = self._o
        try:
    ...
    def build_inline_directive(
        self, base_temperature: float = 0.72,
    ) -> tuple[str, float, float]:
        """
        Returns (prompt_prefix, temperature, presence_penalty).

        Replaces the old blocking Pass 1 call. Temperature/presence_penalty
        come from the real dominant drive (not the LLM's self-report of it),
        and the prefix asks the model to reason inside <think>...</think>
        before answering, within this same generation — one call instead
        of two.
        """
        if not self.enabled:
            return "", base_temperature, 0.2

        drive_name        = self._dominant_drive_name()
        temperature       = self._drive_temperature(drive_name, base_temperature)
        presence_penalty  = self._drive_presence_penalty(drive_name)
        depth_instruction = self._depth_from_energy()
        identity_anchor   = self._get_identity_anchor()

        prefix = (
            f"[Inner process — reason before you speak]\n"
            f"Before answering, think privately inside <think>...</think> tags.\n"
            f"Inside, briefly cover:\n"
            f"INTENT: what the user actually wants or needs\n"
            f"STANCE: what you genuinely think or feel about it\n"
            f"SUPPRESS: what you will NOT say, and why\n"
            f"STYLE: how you'll express it — tone, directness, warmth\n"
            f"Keep each line to 1-2 sentences. Then close </think> and write "
            f"your actual response to the user, informed by that reasoning. "
            f"Never mention this process or show the <think> block.\n\n"
            f"Depth guidance: {depth_instruction}\n"
            f"Identity anchor: {identity_anchor}\n"
            f"[End inner process guidance]\n\n"
        )
        return prefix, temperature, presence_penalty

    def capture_reasoning(self, think_block_text: str) -> None:
        """
        Parse a <think> block captured mid-stream and store it exactly like
        the old Pass 1 result, so last_reasoning()/summary() (read by
        cognitive_validator, etc.) keep working unchanged.
        """
        if not think_block_text or len(think_block_text.strip()) < 10:
            return
        try:
            self._last_reasoning = self._parse_inner_reasoning(think_block_text)
            self._total_passes  += 1
        except Exception as e:
            logger.debug(f"[InnerMonologue] capture_reasoning parse failed: {e}")
