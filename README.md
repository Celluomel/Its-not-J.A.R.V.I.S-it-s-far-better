...
The feature is disabled by default. Set `VOICEMEM_ENABLED` in Memory settings (persisted to `config.json`) only after installing the optional package. To install it during setup, set `LUMINA_INSTALL_VOICEMEM=1` before running `start.bat` or `setup.sh`; model downloads and any upstream provider requirements remain a separate, explicit step. VoiceMem currently declares `transformers==4.52.3`, which conflicts with this project's Coqui-TTS pin (`<=4.46.2`), so the setup scripts install VoiceMem without dependency replacement and restore the Coqui-compatible version. This preserves PandoraBOX's existing audio stack; full VoiceMem audio components may need a separate environment. If VoiceMem is missing, incompatible, or fails at runtime, the native memory and audio systems continue normally. Voice data is sensitive, so local storage and explicit consent are required.
